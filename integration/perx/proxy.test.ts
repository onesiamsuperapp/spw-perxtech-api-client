...
describe('PerxProxyManager', () => {
  ...
  const testableLoyaltyProgramIdOnPerxServer =
    process.env.TEST_PERX_LOYALTY_PROGRAM_ID || ''
  const testableUserIdentifierOnPerxServer =
    process.env.TEST_PERX_USER_IDENTIFIER || ''
  ...
  const testableMerchantIdentifier =
    process.env.TEST_PERX_MERCHANT_IDENTIFIER || ''
  ...
  if (!testableUserIdentifierOnPerxServer) {
    throw new Error(
      'Unable to run test without proper configuration. Please revise your .env file. (in root folder)',
    )
  }
  const manager = new PerxProxyManager(client)
  const userAccount = new PerxLoyaltyTransactionRequestUserAccount({
    type: 'id',
  ...
  describe('user', () => {
    ...
    it('can query categories', async () => {
      ...
      if (hasCategory) {
        ...
        expect(uniq(allCats.map((o) => o.id)).length).toEqual(
  ...
  describe('LoyaltyPoints', () => {
    ...
    it('can query the user points', async () => {
      program = await user.getLoyaltyProgram(
        testableLoyaltyProgramIdOnPerxServer,
      )

      expect(program).toBeTruthy()
      expect(program.pointBalance).toBeDefined()
      reservationBalanceBeforeReserve = program.redemptionInProgressBalance
    })

    it('can added some points in', async () => {
      await pos.submitLoyaltyTransaction(
        new PerxLoyaltyTransactionRequest(
          userAccount,
          +testableLoyaltyProgramIdOnPerxServer,
          pointsToTest,
          null,
      ...
      const newPrg = await user.getLoyaltyProgram(
        testableLoyaltyProgramIdOnPerxServer,
      )
      expect(newPrg).toBeTruthy()
      expect(newPrg.pointBalance - program!.pointBalance).toEqual(pointsToTest)
      expect(newPrg.redemptionInProgressBalance).toEqual(
        reservationBalanceBeforeReserve,
      )
      program = newPrg
    })
    ...
    it('will throws an error when reserving points exceeds current balance', async () => {
      ...
      await expect(() =>
        pos.reserveLoyaltyPoints(
          new PerxLoyaltyTransactionReservationRequest(
            userAccount,
            +testableLoyaltyProgramIdOnPerxServer,
            exceedingPoints,
          ),
    ...
    it('can reserve the loyalty points', async () => {
      const reserved = await pos.reserveLoyaltyPoints(
        new PerxLoyaltyTransactionReservationRequest(
          userAccount,
          +testableLoyaltyProgramIdOnPerxServer,
          pointsToTest,
      ...
      expect(`${reserved.loyaltyProgramId}`).toEqual(
        testableLoyaltyProgramIdOnPerxServer,
      )
    ...
    it('can see the the points has been reserved', async () => {
      ...
      expect(newPrg.redemptionInProgressBalance).toEqual(
        reservationBalanceBeforeReserve + pointsToTest,
      )
    })
  ...
  if (testableRewardId) {
    describe('vouchers', () => {
      ...
      it('cannot be reserved for the second time', async () => {
        await expect(() =>
          user.reserveVouchers([targetVoucherId]),
  ...
  if (testablePerxTriggerId) {
    it('Allow user to trigger with ruleId', async () => {
      await expect(
        user.performCustomTrigger(testablePerxTriggerId),
  ...
  it('Throw error "The record requested does not exist" when trigger bad ruleId', async () => {
    await expect(
      user.performCustomTrigger('some-invalid-rule-id'),
    ).rejects.toThrow(/record requested does not exist/)
  })
  ...
  describe('can list merchants', () => {
    ...
    `('with count $count', async ({ count }) => {
      ...
      expect(
        listMerchants.data.filter((o) => o instanceof PerxMerchant).length,
      ).toEqual(count)
      expect(
        uniq(listMerchants.data.map((o) => o.id).filter(Boolean)).length,
      ).toEqual(count)
  ...
  if (testableRewardId && testableMerchantIds.length >= 2) {
    ...
    `(
      'Invoice for $pointsToUse points on $numberOfTransactions txs using $reserveVoucherBy',
      ({
        pointsToUse,
        ...
        let loyaltyTransactionId = 0
        let targetVoucherId = 0

        if (distribution.length !== numberOfTransactions) {
          throw Error(
            'Invalid test suite setting! numberOfTransactions mismatched distribution array',
          )
        }
        if (voucherUsages.length !== numberOfTransactions) {
          throw Error(
            'Invalid test suite setting! numberOfTransactions mismatched voucherUsages array',
          )
        }
        if (merchantIds.length !== numberOfTransactions) {
          throw Error(
            'Invalid test suite setting! numberOfTransactions mismatched merchantIds array',
          )
        }

        it('prepare loyalty points', async () => {
          const tx = await pos.submitLoyaltyTransaction(
            new PerxLoyaltyTransactionRequest(
              userAccount,
              +testableLoyaltyProgramIdOnPerxServer,
              pointsToUse,
          ...
          expect(tx.loyaltyProgramId).toEqual(
            +testableLoyaltyProgramIdOnPerxServer,
          )
        ...
        if (reserveVoucherBy === 'voucherApi') {
          it('prepare reward to be redeem by Voucher APIs', async () => {
            ...
            const reservedVochers = await user.reserveVouchers([
              `${voucher.id}`,
            ])
            expect(reservedVochers).toBeInstanceOf(Array)
            expect(reservedVochers.length).toEqual(1)
          ...
          it('prepare reward to redeem by Reward APIs', async () => {
            ...
            const issuedVoucher = await user.confirmReservedReward(
              `${voucher.id}`,
            )
        ...
        it('can confirm by single invoice API', async () => {
          ...
          for (let i = 0; i < numberOfTransactions; i++) {
            txs.push(
              new PerxInvoiceRequestTransactionData(3000, merchantIds[i], {
                appliedPoints: distribution[i],
                appliedVouchers: voucherUsages[i] ? [targetVoucherId] : [],
                properties: {
                  test: `MERC: ${merchantIds}, PNTS: ${
                    distribution[i]
                  }, VCHR?: ${voucherUsages[i] ? 'YES' : 'NOPE'}`,
                },
              }),
            )
          }
  ...
  describe('campaign', () => {
    if (testableGameId) {
      ...
      it('can list campaign with campaign type = game', async () => {
        ...
        expect(
          listCampaign.data.filter((o) => o instanceof PerxCampaign).length,
        ).toBeGreaterThan(1)
        expect(
          uniq(listCampaign.data.map((o) => o.id).filter(Boolean)).length,
        ).toBeGreaterThan(1)
        expect(listCampaign.error).toBe(undefined)
        if (campaignIds) {
          const doArraysIntersect = (array1: number[], array2: number[]) =>
